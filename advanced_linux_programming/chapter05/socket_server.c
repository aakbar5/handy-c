...
int main() {
    ...
    ASSERT_POSIX(socket_fd >= 0, "Fail to create a socket");
    ...
    ASSERT_POSIX(ret >= 0, "Fail to bind a socket");

    printf("Listening at port # %d\n", port_number);
    ret = listen(socket_fd, 5);
    ASSERT_POSIX(ret >= 0, "Fail to listen on a socket");

    int client_socket_fd = accept(socket_fd, (struct sockaddr *)&local_addr, &sock_len);
    ASSERT_POSIX(client_socket_fd >= 0, "Fail to accept connection");
