...
int main() {
    ...
    ASSERT_POSIX(shm_id > 0, "Fail to allocate SHM");
    ...
    ASSERT_POSIX(ret >= 0, "Fail to get info SHM");
    ...
    /* Deattach the memory */
    ret = shmdt(memory);
    ASSERT_POSIX(ret >= 0, "Fail to de-attach SHM");

    /* Deallocate the memory */
    ret = shmctl(shm_id, IPC_RMID, 0);
    ASSERT_POSIX(ret >= 0, "Fail to de-allocate SHM");
