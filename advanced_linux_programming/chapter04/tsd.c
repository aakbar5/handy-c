...
static void update_log_file(char *msg) {
    ...
    if (fp) {
        ...
        ASSERT_POSIX(fp == NULL, "Fail to get data from pthread tls");    
    }
}
...
static void* tid_callback(void *arg) {
    ...
    ASSERT_POSIX(ret == 0, "Fail to set data to pthread tls");
...
int main() {
    ...
    ASSERT_POSIX(ret == 0, "Fail to create a pthread key");
