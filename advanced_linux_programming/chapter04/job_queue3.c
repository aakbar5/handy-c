...
static int push_job(void *data) {
    ...
    ASSERT_POSIX(ret == 0, "Fail to lock a mutex");
    ...
    ASSERT_POSIX(ret == 0, "Fail to unlock a mutex");
...
static void* producer_callback(void *arg) {
    ...
    while (1)
    {
        ...
        ASSERT_POSIX(ret == 0, "Fail to wait on a sema");
    }
...
static void* consumer_callback(void *arg) {
    ...
    while (1)
    {
        ...
        if (ret == 0) {
            ...
            ASSERT_POSIX(ret == 0, "Fail to post a sema");
